...
class GlobalCommands:
    ...
    async def _perform_pairing(self, pairing):
        ...
        if pairing.has_paired:
            ...
            print(
                f"You may now use these credentials: {pairing.service.credentials}")
        else:
...
"""
        ...
        if service.pairing == PairingRequirement.NotNeeded:
            print(
                f"Ignoring {service.protocol.name} since pairing is not needed")
            service.credentials = None
            return
...
def _in_range(lower, upper, allow_none=False):
    def _checker(value):
        ...
        raise argparse.ArgumentTypeError(
            f"Must be greater >= {lower} and < {upper}")
...
def _manual_device(args):
    ...
    service.credentials = getattr(
        args, f"{args.protocol.name.lower()}_credentials")
